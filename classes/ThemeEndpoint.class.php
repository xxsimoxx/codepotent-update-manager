...
class ThemeEndpoint {
	...
	public function render_meta_box_primary($post) {
		...
		echo sprintf(
				esc_html__('Use the form %1$stheme-folder-name%2$s to match that of your theme.', 'codepotent-update-manager'),
					'<code>',
					'</code>');
		echo '</p>'."\n";
		echo '			</td>'."\n";
		...
		echo '				<p class="description">';
		echo sprintf(
			esc_html__('Describe the theme as if writing a %1$sreadme.txt%2$s file for it. Basic %3$smarkdown%4$s is supported. HTML and PHP tags are stripped.', 'codepotent-update-manager'),
	...
	public function render_meta_box_autocompleters() {
		...
		echo '<p>';
		echo sprintf(
			esc_html__('%1$sWork smarter!%2$s Click to auto-fill the editor with a premade template you can fill out.', 'codepotent-update-manager'),
			'<strong>',
			'</strong>');
		echo '</p>';
		...
		echo '<p>';
		echo sprintf(
			esc_html__('You can insert a %1$sfully completed example%2$s to get your bearings, or even just see the %3$sabsolute minimum requirements%4$s.', 'codepotent-update-manager'),
